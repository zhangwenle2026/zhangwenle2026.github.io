...
D = json.load(open('order_penetration_data_0831_final.json'))
...
<div class="date">Data as of 数据截至: 2026-08-31 (BRT) | Metropolitan Region | Operating Period: 0825-0831</div>
</div>
""")
...
def bd_table_rows(bds, prefix):
    ...
    for b in bds:
        ...
        rows.append(f'''<tr class="{row_cls}" onclick="toggleDetail('{detail_id}')" style="cursor:pointer"><td>{rank_disp}</td><td>{b["bd"]} ▶</td><td>{b["bdm"]}</td><td>{b["city"]}</td><td class="score-ns">{b["not_signed"]}</td><td class="score-no">{b["not_online"]}</td><td class="score-nop">{b["not_operating"]}</td><td class="score-hv">{hv_disp}</td><td class="score-eb">{eb_disp}</td><td class="score-total"><b>{b["total"]}</b></td><td>{status}</td><td class="prize"></td></tr>''')
        # detail row
        ...
        for l in leads:
            ...
            eb_cell = '+1' if l.get('early') else '-'
            lead_rows += f'<tr><td>{html.escape(str(l["name"]))}</td><td>{DIR_LABEL[l["direction"]]}</td><td>{l["base"]}</td><td>{hv_cell}</td><td>{eb_cell}</td><td><b>{l["score"]}</b></td></tr>\n'
        if lead_rows:
            rows.append(f'''<tr class="detail-row" id="detail-{detail_id}" style="display:none"><td colspan="12"><div class="detail-table-wrap"><table class="detail-table"><thead><tr><th>Lead Name 商家</th><th>Direction 方向</th><th>Base 基础分</th><th>HV 高价值+2</th><th>EB 早鸟+1</th><th>Subtotal 小计</th></tr></thead><tbody>
{lead_rows}</tbody></table></div></td></tr>''')
    return '\n'.join(rows)

TH = '<table><thead><tr><th>Rank 排名</th><th>BD</th><th>BDM</th><th>City 城市</th><th>🔴 Not Sign 未签约</th><th>🟡 Not Online 未上线</th><th>🟢 Not Oper. 未营业</th><th>HV+2 高价值</th><th>EB+1 早鸟</th><th>Total 总分</th><th>Status 达标</th><th>Prize 奖金</th></tr></thead><tbody>'
...
open('order_penetration_dashboard_v5.html', 'w').write('\n'.join(parts))
...
assert '2026-08-31' in content and '0825-0831' in content
assert 'wanessasilva' in content and '59' in content
