#!/usr/bin/env python3
"""Generate DBR daily report card for 2026-08-29 (data date 2026-08-28)."""
import json
import os
import subprocess
from datetime import datetime

# Use latest available data (20260828, since BI lags 1 day)
...
for row in bp_rows:
    ...
    aov = float(row[4]) if row[4] not in ["NaN", "", "NULL"] else 0
    gmv = float(row[5]) if row[5] not in ["NaN", "", "NULL"] else 0
    cancel_rate = float(row[9]) if row[9] not in ["NaN", "", "NULL"] else 0
    mtd_orders = float(row[10]) if row[10] not in ["NaN", "", "NULL"] else 0
    order_wow = float(row[11]) if row[11] not in ["NaN", "", "NULL"] else 0
...
<div class="card">
  <div class="header">
    ...
    <div class="subtitle">DBR Daily Business Report · BRT Aug 29 (Sat) · Data Date: {date_display}, {year}</div>
  </div>
...
html_path = "/mnt/openclaw/.openclaw/workspace/dbr_card_20260829.html"
...
# Screenshot
img_path = "/mnt/openclaw/.openclaw/workspace/dbr_card_20260829.png"
