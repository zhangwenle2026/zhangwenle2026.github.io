...
async def main():
    async with websockets.connect(PAGE_WS) as ws:
        ...
        # If not on controller page, navigate there
        if "dashboard-controller" not in url:
            print("Navigating to controller page...")
            await send_cmd(ws, 10, "Page.navigate", {"url": "https://bi.keetapp.com/v2/dashboard/dashboard-controller?dashboardId=300001446"})
        ...
        # Set date filter using fixed date format instead of OFFSET
        if date_filter_key:
            print(f"\nSetting date filter to fixed date {DATA_DATE}...")
