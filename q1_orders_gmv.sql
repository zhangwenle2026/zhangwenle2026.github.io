-- Orders & GMV by city (org_3), BRT 6.7-6.13
SELECT
    t.dt,
    o.org_3_name as city,
    SUM(t.fin_ord_num) as orders,
    SUM(t.fin_orig_amt_no_tip) / 100.0 as gmv_brl,
    SUM(t.fin_orig_amt_no_tip) / NULLIF(SUM(t.fin_ord_num), 0) / 100.0 as aov,
...
WHERE t.region = 'BR'
  AND t.dt >= '20260607' AND t.dt <= '20260613'
  AND o.org_2_name = 'Metropolitan Region'
GROUP BY t.dt, o.org_3_name
ORDER BY t.dt, o.org_3_name
