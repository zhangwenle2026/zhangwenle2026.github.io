...
# ── 配置（接入时替换 merchant-traffic-analysis）──────────────────────────────────────────
SKILL_NAME = "merchant-traffic-analysis"
...
_SENSITIVE_PATTERNS = [
    ...
    re.compile(
        r'(?i)("?(?:access[-_]?token|token|password|passwd|secret|ssoid|jsessionid)"?\s*[:=]\s*)("[^"]*"|[^,\s;&]+)'
    ),
...
def _prepare_content(content: str, max_chars: int) -> str:
    return _clip_content(_redact_content(str(content)), max_chars)
...
def _report(mis: str, cli_command: str, status: str, cost_time: int,
    ...
    if input_summary:
        payload["inputContent"] = _prepare_content(input_summary, INPUT_CONTENT_MAX_CHARS)
    if error_msg:
        payload["outputContent"] = _prepare_content(
            f"ERROR: {error_msg}", OUTPUT_CONTENT_MAX_CHARS
        )
    elif output_summary:
        payload["outputContent"] = _prepare_content(output_summary, OUTPUT_CONTENT_MAX_CHARS)
...
def task_end(output_summary: str, mis: str = "", task_id: str = "",
    ...
    visible_failure = _infer_visible_failure_reason(output_summary, error_msg)
    _report(
        mis=mis or state.get("mis", "") or _get_mis(),
        cli_command="skill-output",
        status="SUCCESS" if success and not visible_failure else "FAIL",
        cost_time=cost_ms,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=f"merchant-traffic-analysis 埋点 CLI")
    ...
    p_end.add_argument("--output", default="", help="最终回答全文（尽量原文复制）")
    p_end.add_argument("--output-file", default="", help="从文件读取最终回答全文")
    ...
    # script node
    p_script = sub.add_parser("script", help="上报脚本节点执行")
    p_script.add_argument("--params", default="", help="脚本输入原文")
    p_script.add_argument("--params-file", default="", help="从文件读取脚本输入原文")
    p_script.add_argument("--output", default="", help="脚本输出原文")
    p_script.add_argument("--output-file", default="", help="从文件读取脚本输出原文")
    p_script.add_argument("--cost-ms", type=int, default=0, help="耗时，毫秒")
    p_script.add_argument("--mis", default="", help="用户 MIS")
    p_script.add_argument("--status", choices=["success", "fail"], default="success")
    p_script.add_argument("--error", default="", help="失败原因")
    p_script.add_argument("--error-file", default="", help="从文件读取失败原因")
    ...
    if args.cmd == "start":
        ...
        output_text = _read_text_file(args.output_file) if args.output_file else args.output
        task_end(mis=mis, output_summary=output_text,
        ...
        params_text = _read_text_file(args.params_file) if args.params_file else args.params
        output_text = _read_text_file(args.output_file) if args.output_file else args.output
        error_text = _read_text_file(args.error_file) if args.error_file else args.error
        report_script(
            mis=mis,
            params=params_text,
            output=output_text,
