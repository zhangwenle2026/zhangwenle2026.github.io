#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build interactive HTML review page: 43 Western merchants, sortable/filterable/searchable, EN+CN bilingual."""
import json, re

cases = json.load(open('/root/.openclaw/workspace/exclusivity/cases_fresh.json'))
# attitude from case detail (Status do Contrato)
ATT = {}
...
# reclassify by attitude
for c in cases:
    up = money(c.get('upfront'))
    co = num(c.get('comp_orders'))
    et = str(c.get('etapa'))
    if 'aceitou' in et:
        c['cls'] = 'RESOLVED'
    elif c.get('attitude') == 'Assinado':
        c['cls'] = 'LOST'
    elif c.get('attitude') == 'Recebido':
...
for c in sorted(cases, key=lambda c: (ORDER[c['cls']], -money(c.get('upfront')))):
    ...
    if 'aceitou' in et: att = "Rejected comp, chose us 拒绝竞对·选我方"
    elif attitude == 'Assinado': att = "Signed with comp 已签约竞对"
    elif attitude == 'Recebido': att = "Considering offer 竞对报价考虑中"
    else: att = "Not reported 未报状态"
    if 'Encerrado' in et: sit = f"Lost to {c.get('plataforma')} 已失守"
    elif 'aceitou' in et: sit = "Won — sign soon 已胜出待签约"
    elif 'aprovado' in et: sit = "BDM/CM approved, KP next 已批进KP"
    elif up or num(c.get('comp_orders')): sit = f"Under attack 被进攻中"
    else: sit = "Intel missing 情报缺失"
    qs = questions_for(c)
    rows.append({
        'name': c['nome'].strip().strip('|').strip(),
        'id': c.get('merchant_id', ''),
        'plat': PLAT.get(c.get('plataforma'), c.get('plataforma')),
        'bd': c.get('bd', ''),
        'bdm': c.get('bdm', ''),
        'risk': name, 'rk': rk,
        'stage': STAGE.get(et, et),
        'kp': str(c.get('kp')) if str(c.get('kp')) not in ('—', '') else 'Not filled 未录',
        'upfront': up, 'upfront_s': fmt_money(up),
        'co': comp_orders(c), 'ca': comp_aov(c),
        'kgmv': keeta_aov(c), 'k_o': k_o,
        'sit': sit, 'qs': qs, 'upd': str(c.get('updated', ''))[:5],
        'facts': fact_cell, 'att': att,
    })
...
<title>Western Exclusivity Review 西部独家盘点 (2026-09-24)</title>
<style>
...
select { padding:8px 10px; border:1px solid var(--line); border-radius:8px; font-size:13px; background:var(--card); color:var(--txt); max-width:200px; }
...
.att.rejected{background:#d1fae5;color:#047857} .att.signed{background:#e5e7eb;color:#374151} .att.considering{background:#fee2e2;color:#b91c1c} .att.not{background:#f3f4f6;color:#6b7280}
...
<body>
<div class="wrap">
<h1>Western Exclusivity Review 西部独家案件盘点</h1>
<div class="sub">Western São Paulo Metropolitan · 43 merchants 商家 · Data updated 数据更新: 2026-09-23 · Click column headers to sort 点击表头排序</div>
<div class="cards">
 ...
 <div class="card"><div class="n">__UPTOTAL__</div><div class="l">Known Upfront 已知预付</div></div>
</div>
<div class="toolbar">
 ...
 <select id="fPlat"><option value="">Platform 平台: All 全部</option><option>Yellow(99)</option><option>Red(iFood)</option><option>Red+Yellow</option></select>
 <select id="fAtt"><option value="">KP Attitude 态度: All 全部</option><option value="rejected">Rejected comp 拒绝竞对</option><option value="signed">Signed with comp 已签约竞对</option><option value="considering">Considering 考虑中</option><option value="not">Not reported 未报</option></select>
...
<div class="foot">Risk by merchant real status (Status do Contrato) 风险按商家真实状态: Signed with comp=Lost 已签约竞对=已失守 · Considering=High/Medium 考虑中=高/中风险 · Our counter accepted (KP)=Resolved 我方反制通过=已解除 · Questions personalized 追问按商家定制 · Source: SMB Dashboard Exclusividade Tracking</div>
</div>
<script>
const DATA = __DATA__;
...
function render(){
  ...
  let rows = DATA.filter(r=>{
    ...
    const a0=r.att.split(' ').slice(0,2).join(' ').toLowerCase().replace(/[,\s]+$/,''); const attKey = {'rejected comp':'rejected','signed with':'signed','considering offer':'considering','not reported':'not'}[a0]||('considering'===a0?'considering':'');
    if(fa && attKey!==fa) return false;
    if(q){
  ...
  tbody.innerHTML = rows.map(r=>`<tr>
   ...
   <td><span class="att ${r.att.split(' ')[0].toLowerCase()}">${esc(r.att)}</span></td>
   <td>${esc(r.stage)}</td>
   <td>${esc(r.kp)}</td>
...
upt = sum(r['upfront'] for r in rows)
upt_s = ('R$ %.2fM' % (upt/1000000)).replace('.00M','M') if upt >= 1000000 else 'R$ %dK' % (upt//1000)
html = html.replace('__UPTOTAL__', upt_s)
open('/root/.openclaw/workspace/exclusivity/western_review.html', 'w').write(html)
